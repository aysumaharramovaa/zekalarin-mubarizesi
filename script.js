...
function renderCategoryCards() {
    ...
    categoriesList.forEach(cat => {
        ...
        <div onclick="startQuiz('${cat.name}')" 
             class="group bg-slate-900 border border-slate-700 hover:border-${cat.color}-400 rounded-3xl p-8 text-center cursor-pointer hover:-translate-y-3 hover:shadow-xl">
            <div class="text-7xl mb-6">${cat.icon}</div>
            <h3 class="text-3xl font-semibold mb-2">${cat.name}</h3>
            <p class="text-slate-400 text-sm">20 saniyə / sual</p>
        </div>`;
...
function loadCurrentQuestion() {
    ...
    document.getElementById('question-text').innerHTML = `
        <span class="text-cyan-400 block mb-4 text-xl">Sual ${currentIndex + 1}</span>
        ${q.question}
    `;
    ...
    q.options.forEach((option, i) => {
        ...
        div.className = `px-8 py-6 bg-slate-800 hover:bg-slate-700 rounded-3xl text-xl cursor-pointer flex items-center border border-transparent`;
        div.innerHTML = `<span class="font-mono text-cyan-400 w-10">${String.fromCharCode(65 + i)}</span> ${option}`;
...
function finishQuiz() {
    ...
    document.getElementById('score-percentage').parentElement.style.display = 'none';
}
